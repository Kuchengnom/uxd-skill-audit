...
## Das Gedächtnis, und was es wirklich ist

Der Begriff „Memory" auf der Website und im README könnte zu einer Fehlinterpretation führen, die im Ökosystem häufig vorkommt: die Vorstellung, das LLM selbst erinnere sich. Das ist nicht was hier passiert.

Was tatsächlich passiert: Der Skill schreibt nach Abschluss einer Session Designentscheidungen in die Datei `.interface-design/system.md` auf dem Filesystem des Nutzers. Beim Start einer neuen Session lädt Claude Code diese Datei und hat damit Zugriff auf die früheren Entscheidungen. Das LLM bleibt stateless. Verändert hat sich die externe Persistenz der Eingabe.
...
Das ist keine typische Skill-Rhetorik. Die meisten Skills im Ökosystem positionieren sich als Lösungen. Dieser Skill kommuniziert seine Grenzen direkt in der Instruktion, nicht als Fußnote, sondern als strukturell eingeplante Selbstkritik. Der Autor weiß, dass der LLM-Raum für Defaults anfällig ist, und baut das als Wissen direkt in den Prozess ein.

Die vier Checks am Ende des Builds heißen Swap-Test, Squint-Test, Signature-Test und Token-Test. Sie sind ein Beispiel dafür, wie Validierung als Schritt im Workflow verankert werden kann. Kein formaler Testrahmen, aber eine bewusste Unterbrechung des Automatismus.

## Scope-Klarheit als strukturelle Stärke

Die explizite Eingrenzung auf Interface-Design statt Marketing ist mehr als eine Randnotiz. Sie bedeutet: Der Skill hat eine Identität. Er weiß was er ist und was er nicht ist. Im Unterricht lässt sich daraus eine direkte Frage ableiten: Warum brauchen unterschiedliche Design-Kontexte unterschiedliche Skills? Was macht ein Marketingdesign-Skill methodisch anders als ein Interface-Design-Skill?
...
Die berichtete Issue #13, in der Skills und Commands nach der Installation nicht im Slash-Menu erscheinen, ist ein konkreter Bug der seit März 2026 offen ist. Das ist ein technisches Problem das den Einsatz in bestimmten Konfigurationen beeinträchtigen kann. Kein konzeptionelles Problem, aber etwas das bei der Installation berücksichtigt werden sollte.
...
Konkret empfiehlt sich folgender Einsatz: Den SKILL.md lesen lassen, dann eine Aufgabe stellen. „Nennt für jedes Prinzip eine anerkannte Quelle, die es stützt." Das wird längere Recherchearbeit erfordern. Die Prinzipien sind nicht aus der Luft gegriffen, sie finden sich in UI-Literatur, in Designsystem-Dokumentationen großer Unternehmen, in akademischen Schriften zur visuellen Hierarchie. Nur sind sie dort eben nicht verlinkt. Diese Lücke eigenständig zu schließen ist eine wertvolle Übung.

Für den praktischen Einsatz: Wer Dashboards und Admin-Interfaces mit Claude Code baut, bekommt mit diesem Skill ein konsistentes, ernsthaftes Fundament. Der system.md-Mechanismus ist keine Magie, sondern eine gut durchdachte Konvention. Und die eingebettete Selbstkritik an LLM-Defaults ist einer der ehrlichsten Umgänge mit dieser Einschränkung im gesamten Ökosystem.
