...
Genau deshalb lohnt sich der genaue Blick auf den Absatz darunter. Dort steht, die sieben Werkzeuge erhöhten entweder die Ability, nämlich Reduction, Tunneling und Self-Monitoring, oder schärften den Prompt, nämlich Suggestion, Tailoring, Conditioning und Surveillance. Diese Zuordnung stammt nicht von Fogg, sondern vom Autor, und sie ist in zwei Punkten schief. Conditioning, also positive Verstärkung durch Belohnung, wirkt auf die Motivation, nicht auf den Auslöser. Surveillance, die soziale Sichtbarkeit gegenüber Peers, ebenfalls. Beides sind Motivationshebel, und im Fogg-Modell ist Motivation die dritte, hier vollständig unbesetzte Variable. Dazu kommt, dass die sieben Tools aus *Persuasive Technology* von 2003 stammen, während B=MAP die spätere Formulierung des Fogg-Behavior-Modells ist. Zwei Werke, zwei Denkrahmen, ohne Zwischenschritt zusammengeschoben.
...
Zwei Lücken bleiben. Die erste betrifft die Nachprüfbarkeit. Es gibt in diesem Repository kein einziges Beispiel-Output, keinen Before/After-Vergleich, keinen Testfall. Nichts, woran sich prüfen ließe, ob ein Skill leistet, was er ankündigt. Die Ausgabeformate sind zwar streng spezifiziert, mit Ein-Zeilen-Verdikt, Stichpunkten statt Absätzen und maximal drei Priority Actions, was die Form reproduzierbar macht; über die Güte des Inhalts sagt das nichts. Verschärfend kommt hinzu, dass `persuasive-ux.md` zweimal auf `references/tools.md` als Vertiefungsquelle verweist, auf eine Datei die im Repository nicht existiert. Und selbst wenn es sie gäbe, würde `install.sh` sie nicht mitkopieren: Das Skript sammelt ausschließlich `*.md` im Wurzelverzeichnis, Unterordner sind ihm unbekannt. Der Verweis ist also nicht bloß offen, er ist konstruktionsbedingt tot. Ein Modell, das dieser Anweisung folgt, sucht eine Datei, die es nie geben wird.
