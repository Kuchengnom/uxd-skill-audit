...
Manche Skills verkaufen sich über Zahlen, manche über große Namen. `ceorkm/mobile-app-ui-design` verkauft sich über etwas Drittes: über Begriffe, die in der UX-Literatur tatsächlich existieren. Peak-End-Rule. F-Pattern. Thumb Zone. 8-Point-Grid. 60/30/10-Regel. Wer diese Liste liest, erkennt sofort: Hier hat jemand recherchiert, nicht nur Stilrichtungen von Dribbble kopiert. Das macht diesen Skill zu einem interessanten Testfall — nicht weil er besonders groß oder viral ist (51 Stars, 8 Forks, vier Commits seit der Veröffentlichung), sondern weil er eine Frage zuspitzt, die in dieser Serie immer wieder auftaucht: Reicht es, die richtigen Begriffe zu nennen?

## Was der Skill ist

Der Skill besteht aus einer rund 150 Zeilen langen `SKILL.md` und einer Referenzdatei `industry-conventions.md` für branchenspezifische Muster (Fintech, Health, Crypto und weitere). Er führt durch einen Fünf-Schritte-Prozess — Kontext verstehen, Struktur vor Optik, visuelles Design, emotionales Design, Polish — und endet mit konkreten Implementierungshinweisen für React, Tailwind CSS und Lucide-Icons. Lizenziert unter MIT. Autor ist ceorkm, ein Entwickler mit sichtbarem Portfolio an KI-Tooling-Projekten, darunter `grably` (knapp 300 Stars), `kratos-mcp` und `reactbits-mcp-server`. Das ist kein anonymer Wegwerf-Account, sondern jemand mit nachvollziehbarer Entwicklungsgeschichte im Ökosystem.

## Die richtigen Begriffe, aber ohne Beleg

<div class="callout">
<div class="callout__title">Der Unterschied zwischen Kennen und Belegen</div>
Die Peak-End-Rule stammt aus Daniel Kahnemans Forschung zum retrospektiven Erleben — Arbeit, für die er den Nobelpreis erhielt. Der Skill nennt sie korrekt beim Namen. Aber nirgendwo in der SKILL.md steht "Kahneman". Kein Link, kein Jahr, keine Studie. Dasselbe gilt für das F-Pattern (Eyetracking-Forschung, ursprünglich von der Nielsen Norman Group, seit Jahren in der eigenen Fachwelt umstritten) und die Thumb-Zone-Heuristik (ergonomische Feldstudien, unter anderem von Steven Hoober). Alle drei Konzepte sind real. Keines davon ist im Skill nachvollziehbar verlinkt.
</div>

Das ist ein anderes Problem als bei Skills, die Quellen frei erfinden oder gar nicht erst erwähnen. Hier sind die Begriffe korrekt — aber sie funktionieren im Text wie Gütesiegel, nicht wie Referenzen. Man liest "Peak-End Rule" und nimmt automatisch an, dass dahinter etwas Geprüftes steht. Das stimmt sogar — nur eben nicht in diesem Dokument. Wer nachschlagen möchte, ob die im Skill formulierte Anwendung ("Identifiziere deinen Peak-Moment: Abschluss einer Kernaufgabe, Erreichen eines Meilensteins") der Forschung von Kahneman wirklich entspricht oder eine stark vereinfachte Faustregel daraus macht, hat keine Spur, der er folgen kann.
...
Dieser Skill eignet sich gut als Einstiegsübung für eine sehr konkrete Aufgabe: Studierende sollen jeden im Skill genannten Fachbegriff einzeln nachschlagen, die Originalquelle identifizieren und prüfen, ob die im Skill formulierte Anwendung der Quelle entspricht oder sie vereinfacht. Das lässt sich hier gut durchführen, weil die Begriffe real und auffindbar sind — anders als bei Skills mit erfundenen oder vagen Quellenbehauptungen, wo die Übung ins Leere läuft.
